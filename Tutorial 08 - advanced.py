...
CONST_STEP = 49
CONST_TEST_CASES = 245


def dataNormalization(data):
    return [(datum -  data[0]) / data[0].T for datum in data]


def dataDeNormalization(data, base):
    return [(datum + 1) * base.T for datum in data]


def getDeepLearningData(ticker):
    # Step 1. Load data
    data = pandas.read_csv('../stock_data/01. IntradayCN/' + ticker + '.csv')[{'close', 'vol'}].as_matrix()
    # Step 2. BUilding Training data
    dataTraining = []
    print('data.shape:', data.shape, 'len(data)', len(data))
    for i in range(0, len(data) - CONST_TEST_CASES, CONST_STEP):
        dataSegment = data[i:i+CONST_STEP]
...
def predictLSTM(ticker):
    ...
    # # Step 3. Train model
    # model.fit(X_Training, Y_Training, batch_size=512, nb_epoch=5, validation_split=0.05)
    #
    # # Step 4. Predict
